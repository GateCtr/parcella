...
router.post("/fiches/:id/plaque", async (req,res):Promise<void>=>{
  ...
  if(existing.length&&["generee","a_reimprimer"].includes(f.statutPlaque)&&existing[0].svg.includes('id="plaque-layout-v4"')){res.status(409).json({error:"Plaque déjà générée"});return;}
